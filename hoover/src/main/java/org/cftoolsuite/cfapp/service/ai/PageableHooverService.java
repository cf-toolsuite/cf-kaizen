...
public class PageableHooverService {
	...
	public Page<AppDetail> getPageableApplications(
		...
		List<AppDetail> applications = snapshotDetail != null
				? snapshotDetail.getApplications()
	...
	public Page<String> getPageableServiceAccounts(
		...
		SnapshotDetail snapshotDetail = hooverApiClient.snapshotDetailGet().getBody();
		List<String> serviceAccounts = snapshotDetail != null
				? snapshotDetail.getServiceAccounts()
				: Collections.emptyList();
	...
	public Page<Organization> getPageableFilteredOrganizations(
		...
		if (StringUtils.isNotBlank(namePattern) && allOrganizations != null) {
			...
			allOrganizations = allOrganizations.stream().filter(
					org -> StringUtils.isNotBlank(org.getName()) && org.getName().toLowerCase().contains(pattern))
	...
	public Page<Space> getPageableFilteredSpaces(
		...
		if (allSpaces != null) {
			if (StringUtils.isNotBlank(organization)) {
				allSpaces = allSpaces.stream().filter(s -> organization.equalsIgnoreCase(s.getOrganizationName()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(namePattern)) {
				String pattern = namePattern.toLowerCase();
				allSpaces = allSpaces.stream().filter(s -> StringUtils.isNotBlank(s.getSpaceName())
						&& s.getSpaceName().toLowerCase().contains(pattern)).collect(Collectors.toList());
			}
		}
	...
	public Page<AppDetail> getPageableFilteredApplications(
		...
		if (!CollectionUtils.isEmpty(applications)) {
			if (StringUtils.isNotBlank(organization)) {
				applications = applications.stream().filter(app -> organization.equalsIgnoreCase(app.getOrganization()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(space)) {
				applications = applications.stream().filter(app -> space.equalsIgnoreCase(app.getSpace()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(namePattern)) {
				String pattern = namePattern.toLowerCase();
				applications = applications.stream().filter(app -> StringUtils.isNotBlank(app.getAppName())
						&& app.getAppName().toLowerCase().contains(pattern)).collect(Collectors.toList());
			}
		}
	...
	/**
	 * Get a pageable filtered list of Spring Boot applications based on various
	 * criteria.
	 */
	...
	public Page<JavaAppDetail> getPageableFilteredSpringApplications(
		...
		if (!CollectionUtils.isEmpty(allJavaApps)) {
			if (StringUtils.isNotBlank(organization)) {
				allJavaApps = allJavaApps.stream().filter(app -> organization.equalsIgnoreCase(app.getOrganization()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(space)) {
				allJavaApps = allJavaApps.stream().filter(app -> space.equalsIgnoreCase(app.getSpace()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(namePattern)) {
				String pattern = namePattern.toLowerCase();
				allJavaApps = allJavaApps.stream().filter(app -> StringUtils.isNotBlank(app.getAppName())
						&& app.getAppName().toLowerCase().contains(pattern)).collect(Collectors.toList());
			}
	...
	public Page<ServiceInstanceDetail> getPageableFilteredServiceInstances(
		...
		if (!CollectionUtils.isEmpty(serviceInstances)) {
			...
			if (StringUtils.isNotBlank(space)) {
				serviceInstances = serviceInstances.stream().filter(svc -> space.equalsIgnoreCase(svc.getSpace()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(namePattern)) {
				String pattern = namePattern.toLowerCase();
				serviceInstances = serviceInstances.stream().filter(
						svc -> StringUtils.isNotBlank(svc.getName()) && svc.getName().toLowerCase().contains(pattern))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(serviceOffering)) {
				serviceInstances = serviceInstances.stream()
						.filter(svc -> serviceOffering.equalsIgnoreCase(svc.getService())).collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(servicePlan)) {
				serviceInstances = serviceInstances.stream().filter(svc -> servicePlan.equalsIgnoreCase(svc.getPlan()))
						.collect(Collectors.toList());
			}
		}
	...
	/**
	 * Get a pageable filtered list of space users by foundation, organization,
	 * and/or space.
	 */
	...
	public Page<SpaceUsers> getPageableFilteredSpaceUsers(
		...
		if (!CollectionUtils.isEmpty(allSpaceUsers)) {
			if (StringUtils.isNotBlank(foundation)) {
				allSpaceUsers = allSpaceUsers.stream().filter(su -> foundation.equalsIgnoreCase(su.getFoundation()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(organization)) {
				allSpaceUsers = allSpaceUsers.stream().filter(su -> organization.equalsIgnoreCase(su.getOrganization()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(space)) {
				allSpaceUsers = allSpaceUsers.stream().filter(su -> space.equalsIgnoreCase(su.getSpace()))
						.collect(Collectors.toList());
			}
		}
