...
public class ProductsService {
	...
	public Product getProductByName(
			@ToolParam(description = "Product name pattern to search for (case-insensitive)") String namePattern) {
		return Optional.ofNullable(productsApiClient.storeProductCatalogGet()).map(HttpEntity::getBody)
	...
	@Tool(name = "ProductGetLatestProductReleaseBySlug", description = """
			...
			""")
	public Release getLatestProductReleaseBySlug(
			@ToolParam(description = "Product slug identifier pattern (case-insensitive)") String slugPattern) {
		return Optional.ofNullable(productsApiClient.storeProductReleasesGet("latest")).map(HttpEntity::getBody)
