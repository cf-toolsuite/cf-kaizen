...
public class PoliciesService {
	...
	@Tool(name = "PolicyGetApplicationPolicyById", description = """
			...
			""")
	public ApplicationPolicy getApplicationPolicyById(
			@ToolParam(description = "ID of the application policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesApplicationIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getApplicationPolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}

	@Tool(name = "PolicyGetEndpointPolicyById", description = """
	...
	public EndpointPolicy getEndpointPolicyById(@ToolParam(description = "ID of the endpoint policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesEndpointIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getEndpointPolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}
	...
	@Tool(name = "PolicyListAllPolicies", description = """
	...
	@Tool(name = "PolicyGetHygienePolicyById", description = """
	...
	public HygienePolicy getHygienePolicyById(@ToolParam(description = "ID of the hygiene policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesHygieneIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getHygienePolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}

	@Tool(name = "PolicyGetLegacyPolicyById", description = """
	...
	public LegacyPolicy getLegacyPolicyById(@ToolParam(description = "ID of the legacy policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesLegacyIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getLegacyPolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}

	@Tool(name = "PolicyGetQueryPolicyById", description = """
	...
	public QueryPolicy getQueryPolicyById(@ToolParam(description = "ID of the query policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesQueryIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getQueryPolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}
	...
	public String getPoliciesReport(@ToolParam(description = "Start date for the report (YYYY-MM-DD).") LocalDate start,
	...
	@Tool(name = "PolicyGetServiceInstancePolicyById", description = """
			...
			""")
	public ServiceInstancePolicy getPolicyServiceInstanceById(
			@ToolParam(description = "ID of the service instance policy.") String id) {
		return Optional.ofNullable(policiesApiClient.policiesServiceInstanceIdGet(id)).map(HttpEntity::getBody)
				.map(Policies::getServiceInstancePolicies).filter(policies -> !policies.isEmpty()).map(List::getFirst)
				.orElse(null);
	}
}