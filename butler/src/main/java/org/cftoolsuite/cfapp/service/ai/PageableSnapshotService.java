...
public class PageableSnapshotService {
	...
	public Page<AppDetail> getPageableDormantApplications(
		...
		Workloads dormantWorkloads = snapshotApiClient.snapshotDetailDormantDaysSinceLastUpdateGet(daysSinceLastUpdate)
				.getBody();
		List<AppDetail> applications = dormantWorkloads != null
				? dormantWorkloads.getApplications()
	...
	public Page<AppDetail> getPageableApplicationsWithLegacyStack(
		...
		List<AppDetail> applications = legacyWorkloads != null
				? legacyWorkloads.getApplications()
				: Collections.emptyList();
		return createPage(applications, pageNumber, pageSize);
	}
	...
	public Page<AppDetail> getPageableFilteredApplications(
		...
		if (!CollectionUtils.isEmpty(applications)) {
			...
			if (StringUtils.isNotBlank(buildpack)) {
				applications = applications.stream().filter(app -> buildpack.equals(app.getBuildpack()))
						.collect(Collectors.toList());
			}

			if (StringUtils.isNotBlank(stack)) {
				applications = applications.stream().filter(app -> stack.equals(app.getStack()))
						.collect(Collectors.toList());
			}
		}
	...
	public Page<AppRelationship> getPageableFilteredRelationships(
		...
		if (!CollectionUtils.isEmpty(relationships)) {
			if (StringUtils.isNotBlank(applicationName)) {
				relationships = relationships.stream().filter(rel -> applicationName.equalsIgnoreCase(rel.getAppName()))
						.collect(Collectors.toList());
			}
		}
	...
	public Page<String> getPageableFilteredUserAccounts(
		...
		if (StringUtils.isNotBlank(namePattern) && !CollectionUtils.isEmpty(userAccounts)) {
			String pattern = namePattern.toLowerCase();
			userAccounts = userAccounts.stream().filter(name -> name.toLowerCase().contains(pattern))
					.collect(Collectors.toList());
		}
